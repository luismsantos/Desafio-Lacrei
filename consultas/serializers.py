...
class ConsultaSerializer(serializers.ModelSerializer):
    ...
    def validate_paciente_nome(self, value):
        value = value.strip()
        if not value:
            raise serializers.ValidationError("O nome do paciente não pode estar vazio.")
        return value

    def validate_data_hora(self, value):
        if value < timezone.now():
            raise serializers.ValidationError("Não é permitido cadastrar consultas para datas no passado.")
        return value

    def validate_observacoes(self, value):
        if value is not None and value.strip() == "":
            raise serializers.ValidationError("Observações, se informadas, não podem estar vazias.")
        return value
...
class ConsultaDetalheSerializer(serializers.ModelSerializer):
    ...
    profissional_especialidade = serializers.CharField(source='profissional.especialidade')
